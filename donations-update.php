...
if(isset($_POST["DonationID"]) && !empty($_POST["DonationID"])){
    ...
    if (!in_array(true, array_column($upload_results, 'error'))) {
        ...
        $stmt = $conn->prepare("UPDATE `donations` SET `CampaignID`=?,`DonorID`=?,`Amount`=?,`DonationDate`=? WHERE `DonationID`=?");

        try {
            $stmt->execute([ $CampaignID, $DonorID, $Amount, $DonationDate, $DonationID  ]);
        } catch (Exception $e) {
        ...
        if (!isset($error)){
            header("location: donations-read.php?DonationID=$DonationID");
        } else {
...
if(isset($_GET["DonationID"]) && !empty($_GET["DonationID"])){
    // Get URL parameter
    $DonationID =  trim($_GET["DonationID"]);
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            if(mysqli_num_rows($result) == 1){
                ...
                $CampaignID = htmlspecialchars($row["CampaignID"] ?? "");
					$DonorID = htmlspecialchars($row["DonorID"] ?? "");
					$Amount = htmlspecialchars($row["Amount"] ?? "");
					$DonationDate = htmlspecialchars($row["DonationDate"] ?? "");
            ...
            <div class="row">
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post" enctype="multipart/form-data">
                        ...
                        <hr>
                        <p>
                            <a href="donations-read.php?DonationID=<?php echo $_GET["DonationID"];?>" class="btn btn-info"><?php translate('View Record') ?></a>
