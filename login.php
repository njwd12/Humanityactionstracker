...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($email) || empty($password)) {
        ...
        $stmt = $conn->prepare("SELECT UserID, username, password, role, is_verified FROM users WHERE email = ?");
        $stmt->bind_param("s", $email);
        $stmt->execute();
        $stmt->store_result();

        if ($stmt->num_rows > 0) {
            $stmt->bind_result($user_id, $username_db, $db_password, $role, $is_verified);
            $stmt->fetch();

            if (password_verify($password, $db_password)) {
                if ($is_verified == 1) {  // Проверка дали email е верифициран
