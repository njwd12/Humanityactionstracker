...
if(isset($_POST["BeneficiaryID"]) && !empty($_POST["BeneficiaryID"])){
    ...
    if (!in_array(true, array_column($upload_results, 'error'))) {
		...
		$AllocationAmount = $_POST["AllocationAmount"] == "" ? null : trim($_POST["AllocationAmount"]);
		

        // Prepare an update statement

        $stmt = $conn->prepare("UPDATE `campaignbeneficiaries` SET `CampaignID`=?,`BeneficiaryID`=?,`AllocationAmount`=? WHERE `BeneficiaryID`=?");

        try {
            $stmt->execute([ $CampaignID, $BeneficiaryID, $AllocationAmount, $BeneficiaryID  ]);
        } catch (Exception $e) {
        ...
        if (!isset($error)){
            header("location: campaignbeneficiaries-read.php?BeneficiaryID=$BeneficiaryID");
        } else {
...
if(isset($_GET["BeneficiaryID"]) && !empty($_GET["BeneficiaryID"])){
    ...
    $sql = "SELECT * FROM `campaignbeneficiaries` WHERE `BeneficiaryID` = ?";
    if($stmt = mysqli_prepare($conn, $sql)){
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            if(mysqli_num_rows($result) == 1){
                ...
                $CampaignID = htmlspecialchars($row["CampaignID"] ?? "");
					$BeneficiaryID = htmlspecialchars($row["BeneficiaryID"] ?? "");
					$AllocationAmount = htmlspecialchars($row["AllocationAmount"] ?? "");
            ...
            <div class="row">
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post" enctype="multipart/form-data">
						...
						<div class="form-group">
                                            <label for="BeneficiaryID">BeneficiaryID*</label>
                                            
                                        </div>
						<div class="form-group">
                                            <label for="AllocationAmount">AllocationAmount</label>
                                            <input type="number" name="AllocationAmount" id="AllocationAmount" class="form-control" value="<?php echo @$AllocationAmount; ?>" step="any">
                                        </div>
                        ...
                        <hr>
                        <p>
                            <a href="campaignbeneficiaries-read.php?BeneficiaryID=<?php echo $_GET["BeneficiaryID"];?>" class="btn btn-info"><?php translate('View Record') ?></a>
