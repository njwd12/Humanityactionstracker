...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if (!in_array(true, array_column($upload_results, 'error'))) {
        ...
        $stmt = $conn->prepare("INSERT INTO `users` (`FullName`, `Email`, `PasswordHash`, `Role`, `CreatedAt`) VALUES (?, ?, ?, ?, ?)");

        try {
            $stmt->execute([ $FullName, $Email, $PasswordHash, $Role, $CreatedAt ]);
        } catch (Exception $e) {
        ...
        if (!isset($error)){
            ...
            header("location: users-read.php?UserID=$new_id");
        } else {
