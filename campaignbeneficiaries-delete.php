...
if(isset($_POST["BeneficiaryID"]) && !empty($_POST["BeneficiaryID"])){
    ...
    $fileColumns = [];
    if (isset($tables_and_columns_names['campaignbeneficiaries']['columns'])) {
        foreach ($tables_and_columns_names['campaignbeneficiaries']['columns'] as $columnName => $columnDetails) {
    ...
    if (!empty($fileColumns)) {
        foreach ($fileColumns as $columnName) {
            ...
            if ($stmt = mysqli_prepare($conn, $sql)) {
                // Set parameters
                $param_id = trim($_POST["BeneficiaryID"]);
    ...
    $sql = "DELETE FROM `campaignbeneficiaries` WHERE `BeneficiaryID` = ?";
    ...
    if($stmt = mysqli_prepare($conn, $sql)){
        ...
        if (!isset($error)){
            ...
            header("location: campaignbeneficiaries-index.php");
        }
    }
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) . "?BeneficiaryID=" . $_GET["BeneficiaryID"]; ?>" method="post">
                    <?php print_error_if_exists(@$error); ?>
                        <div class="alert alert-danger fade-in">
                            <input type="hidden" name="BeneficiaryID" value="<?php echo trim($_GET["BeneficiaryID"]); ?>"/>
