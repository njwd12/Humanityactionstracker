...
if(isset($_POST["DonationID"]) && !empty($_POST["DonationID"])){
    ...
    $fileColumns = [];
    if (isset($tables_and_columns_names['donations']['columns'])) {
        foreach ($tables_and_columns_names['donations']['columns'] as $columnName => $columnDetails) {
    ...
    if (!empty($fileColumns)) {
        foreach ($fileColumns as $columnName) {
            ...
            if ($stmt = mysqli_prepare($conn, $sql)) {
                // Set parameters
                $param_id = trim($_POST["DonationID"]);
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) . "?DonationID=" . $_GET["DonationID"]; ?>" method="post">
                    <?php print_error_if_exists(@$error); ?>
                        <div class="alert alert-danger fade-in">
                            <input type="hidden" name="DonationID" value="<?php echo trim($_GET["DonationID"]); ?>"/>
