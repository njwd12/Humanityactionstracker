...
if(isset($_POST["CampaignID"]) && !empty($_POST["CampaignID"])){
    ...
    if (!in_array(true, array_column($upload_results, 'error'))) {
		...
		$EndDate = $_POST["EndDate"] == "" ? null : trim($_POST["EndDate"]);
		$TargetAmount = trim($_POST["TargetAmount"]);
		$OrganizerID = trim($_POST["OrganizerID"]);
        ...
        $stmt = $conn->prepare("UPDATE `campaigns` SET `Name`=?,`Description`=?,`StartDate`=?,`EndDate`=?,`TargetAmount`=?,`OrganizerID`=?,`Status`=?,`CreatedAt`=? WHERE `CampaignID`=?");

        try {
            $stmt->execute([ $Name, $Description, $StartDate, $EndDate, $TargetAmount, $OrganizerID, $Status, $CreatedAt, $CampaignID  ]);
        } catch (Exception $e) {
        ...
        if (!isset($error)){
            header("location: campaigns-read.php?CampaignID=$CampaignID");
        } else {
...
$_GET["CampaignID"] = trim($_GET["CampaignID"]);
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            if(mysqli_num_rows($result) == 1){
                ...
                $Name = htmlspecialchars($row["Name"] ?? "");
					...
					$TargetAmount = htmlspecialchars($row["TargetAmount"] ?? "");
                ...
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                                            ...
                                            <input type="text" name="Name" id="Name" maxlength="150" class="form-control" value="<?php echo @$Name; ?>">
                                        </div>
						<div class="form-group">
						...
						<div class="form-group">
                                            ...
                                            <input type="number" name="TargetAmount" id="TargetAmount" class="form-control" value="<?php echo @$TargetAmount; ?>" step="any">
                                        </div>
						<div class="form-group">
						...
						<div class="form-group">
                                            ...
                                            <select name="Status" class="form-control" id="Status"><option value="">Null</option><?php 
							 $enum_Status = array('active','completed','cancelled');
                        ...
                        <hr>
                        <p>
                            <a href="campaigns-read.php?CampaignID=<?php echo $_GET["CampaignID"];?>" class="btn btn-info"><?php translate('View Record') ?></a>
                            <a href="campaigns-delete.php?CampaignID=<?php echo $_GET["CampaignID"];?>" class="btn btn-danger"><?php translate('Delete Record') ?></a>
                            <a href="campaigns-index.php" class="btn btn-primary"><?php translate('Back to List') ?></a>
                        </p>
