...
if(isset($_POST["CampaignID"]) && !empty($_POST["CampaignID"])){
    ...
    $fileColumns = [];
    if (isset($tables_and_columns_names['campaigns']['columns'])) {
        foreach ($tables_and_columns_names['campaigns']['columns'] as $columnName => $columnDetails) {
    ...
    if (!empty($fileColumns)) {
        foreach ($fileColumns as $columnName) {
            ...
            if ($stmt = mysqli_prepare($conn, $sql)) {
                // Set parameters
                $param_id = trim($_POST["CampaignID"]);
    ...
    if($stmt = mysqli_prepare($conn, $sql)){
        ...
        if (!isset($error)){
            ...
            header("location: campaigns-index.php");
        }
    }
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) . "?CampaignID=" . $_GET["CampaignID"]; ?>" method="post">
                    <?php print_error_if_exists(@$error); ?>
                        <div class="alert alert-danger fade-in">
                            <input type="hidden" name="CampaignID" value="<?php echo trim($_GET["CampaignID"]); ?>"/>
                    ...
                    <p>
                        ...
                        <a href="campaigns-update.php?CampaignID=<?php echo $_GET["CampaignID"];?>" class="btn btn-warning"><?php translate('Update Record') ?></a>
