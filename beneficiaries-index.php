...
$total_pages_sql = "SELECT COUNT(*) FROM `beneficiaries`";
$result = mysqli_query($conn,$total_pages_sql);
$total_rows = mysqli_fetch_array($result)[0];
$total_pages = ceil($total_rows / $no_of_records_per_page);

//Column sorting on column name
$columns = array('BeneficiaryID', 'FullName', 'ContactInfo', 'Address', 'Description', 'CreatedAt');
...
if (!empty($_GET['search'])) {
    ...
    if (strpos('`beneficiaries`.`BeneficiaryID`, `beneficiaries`.`FullName`, `beneficiaries`.`ContactInfo`, `beneficiaries`.`Address`, `beneficiaries`.`Description`, `beneficiaries`.`CreatedAt`', ',')) {
        $where_statement .= " AND CONCAT_WS (`beneficiaries`.`BeneficiaryID`, `beneficiaries`.`FullName`, `beneficiaries`.`ContactInfo`, `beneficiaries`.`Address`, `beneficiaries`.`Description`, `beneficiaries`.`CreatedAt`) LIKE '%$search%'";
    } else {
        $where_statement .= " AND `beneficiaries`.`BeneficiaryID`, `beneficiaries`.`FullName`, `beneficiaries`.`ContactInfo`, `beneficiaries`.`Address`, `beneficiaries`.`Description`, `beneficiaries`.`CreatedAt` LIKE '%$search%'";
    }
...
$group_clause = !empty($order) && $order == 'BeneficiaryID' ? "GROUP BY `beneficiaries`.`$order`" : '';
...
if ($isCsvExport) {
    ...
    exportAsCSV($data, $db_name, $tables_and_columns_names, 'beneficiaries', $conn, false);
    exit;
}
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header clearfix">
                        ...
                        <a href="beneficiaries-create.php" class="btn btn-success float-right"><?php translate('Add New Record') ?></a>
                        <a href="beneficiaries-index.php" class="btn btn-info float-right mr-2"><?php translate('Reset View') ?></a>
                        <a href="beneficiaries-index.php?export=csv" class="btn btn-primary float-right mr-2"><?php translate('Export as CSV') ?></a>
                        ...
                        if($result) :
                            if(mysqli_num_rows($result) > 0) :
                                ...
                                <table class='table table-bordered table-striped'>
									...
									$sort_link= isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
									$sort_link= isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_conn;
									echo "<th><a href=?search=$search&order=ContactInfo&sort=".$sort_conn.">ContactInfo</a></th>";
									$columnname = "Address";
									$sort_link= isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
									$sort_link= isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_conn;
									echo "<th><a href=?search=$search&order=Address&sort=".$sort_conn.">Address</a></th>";
 ...
 ?>
                                    ...
                                    <tbody>
                                        <?php while($row = mysqli_fetch_array($result)): ?>
                                            <tr>
                                                <?php echo "<td>" . htmlspecialchars($row['BeneficiaryID'] ?? "") . "</td>";
																					echo "<td>";
											...
											echo nl2br(htmlspecialchars($row['ContactInfo'] ?? ""));
											if ($has_conn_file){
											    echo $is_file ? "</a>" : "";
											}
											echo "</td>"."\n\t\t\t\t\t\t\t\t\t\t\t\t";											echo "<td>";
										 ...
										 ?>
                                                <td>
                                                    ...
                                                    if (!empty($column_id)): ?>
                                                        <a id='read-<?php echo $row['BeneficiaryID']; ?>' href='beneficiaries-read.php?BeneficiaryID=<?php echo $row['BeneficiaryID']; ?>' title='<?php echo addslashes(translate('View Record', false)); ?>' data-toggle='tooltip' class='btn btn-sm btn-info'><i class='far fa-eye'></i></a>
                                                        <a id='update-<?php echo $row['BeneficiaryID']; ?>' href='beneficiaries-update.php?BeneficiaryID=<?php echo $row['BeneficiaryID']; ?>' title='<?php echo addslashes(translate('Update Record', false)); ?>' data-toggle='tooltip' class='btn btn-sm btn-warning'><i class='far fa-edit'></i></a>
                                                        <a id='delete-<?php echo $row['BeneficiaryID']; ?>' href='beneficiaries-delete.php?BeneficiaryID=<?php echo $row['BeneficiaryID']; ?>' title='<?php echo addslashes(translate('Delete Record', false)); ?>' data-toggle='tooltip' class='btn btn-sm btn-danger'><i class='far fa-trash-alt'></i></a>
                                                    <?php else: ?>
