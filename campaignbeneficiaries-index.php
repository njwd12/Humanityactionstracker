...
$total_pages_sql = "SELECT COUNT(*) FROM `campaignbeneficiaries`";
$result = mysqli_query($conn,$total_pages_sql);
$total_rows = mysqli_fetch_array($result)[0];
$total_pages = ceil($total_rows / $no_of_records_per_page);

//Column sorting on column name
$columns = array('CampaignID', 'BeneficiaryID', 'AllocationAmount');
...
if (!empty($_GET['search'])) {
    ...
    if (strpos('`campaignbeneficiaries`.`CampaignID`, `campaignbeneficiaries`.`BeneficiaryID`, `campaignbeneficiaries`.`AllocationAmount`', ',')) {
        $where_statement .= " AND CONCAT_WS (`campaignbeneficiaries`.`CampaignID`, `campaignbeneficiaries`.`BeneficiaryID`, `campaignbeneficiaries`.`AllocationAmount`) LIKE '%$search%'";
    } else {
        $where_statement .= " AND `campaignbeneficiaries`.`CampaignID`, `campaignbeneficiaries`.`BeneficiaryID`, `campaignbeneficiaries`.`AllocationAmount` LIKE '%$search%'";
    }
...
$group_clause = !empty($order) && $order == 'BeneficiaryID' ? "GROUP BY `campaignbeneficiaries`.`$order`" : '';
...
if ($isCsvExport) {
    ...
    exportAsCSV($data, $db_name, $tables_and_columns_names, 'campaignbeneficiaries', $conn, false);
    exit;
}
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header clearfix">
                        ...
                        <a href="campaignbeneficiaries-create.php" class="btn btn-success float-right"><?php translate('Add New Record') ?></a>
                        <a href="campaignbeneficiaries-index.php" class="btn btn-info float-right mr-2"><?php translate('Reset View') ?></a>
                        <a href="campaignbeneficiaries-index.php?export=csv" class="btn btn-primary float-right mr-2"><?php translate('Export as CSV') ?></a>
                        <a href="javascript:history.back()" class="btn btn-secondary float-right mr-2"><?php translate('Back') ?></a>
                    </div>

                    <div class="form-row">
                        <form action="campaignbeneficiaries-index.php" method="get">
                        ...
                        if($result) :
                            if(mysqli_num_rows($result) > 0) :
                                ...
                                <table class='table table-bordered table-striped'>
									...
									$sort_link= isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
									$sort_link= isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_conn;
									echo "<th><a href=?search=$search&order=BeneficiaryID&sort=".$sort_conn.">BeneficiaryID</a></th>";
									$columnname = "AllocationAmount";
									$sort_link= isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
									$sort_link= isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_conn;
									echo "<th><a href=?search=$search&order=AllocationAmount&sort=".$sort_conn.">AllocationAmount</a></th>";
 ?>
                                    ...
                                    <tbody>
                                        <?php while($row = mysqli_fetch_array($result)): ?>
                                            <tr>
                                                <?php echo "<td>" . get_fk_url($row["CampaignID"], "campaigns", "CampaignID", $row["CampaignIDcampaignsCampaignID"], 1, true) . "</td>";
											echo "<td>" . get_fk_url($row["BeneficiaryID"], "beneficiaries", "BeneficiaryID", $row["BeneficiaryIDbeneficiariesBeneficiaryID"], 1, true) . "</td>";
											echo "<td>" . htmlspecialchars($row['AllocationAmount'] ?? "") . "</td>";
										 ?>
                                                <td>
                                                    <?php
                                                    $column_id = 'BeneficiaryID';
                                                    if (!empty($column_id)): ?>
                                                        <a id='read-<?php echo $row['BeneficiaryID']; ?>' href='campaignbeneficiaries-read.php?BeneficiaryID=<?php echo $row['BeneficiaryID']; ?>' title='<?php echo addslashes(translate('View Record', false)); ?>' data-toggle='tooltip' class='btn btn-sm btn-info'><i class='far fa-eye'></i></a>
                                                        <a id='update-<?php echo $row['BeneficiaryID']; ?>' href='campaignbeneficiaries-update.php?BeneficiaryID=<?php echo $row['BeneficiaryID']; ?>' title='<?php echo addslashes(translate('Update Record', false)); ?>' data-toggle='tooltip' class='btn btn-sm btn-warning'><i class='far fa-edit'></i></a>
                                                        <a id='delete-<?php echo $row['BeneficiaryID']; ?>' href='campaignbeneficiaries-delete.php?BeneficiaryID=<?php echo $row['BeneficiaryID']; ?>' title='<?php echo addslashes(translate('Delete Record', false)); ?>' data-toggle='tooltip' class='btn btn-sm btn-danger'><i class='far fa-trash-alt'></i></a>
                                                    <?php else: ?>
