...
if(isset($_POST["ExpenseID"]) && !empty($_POST["ExpenseID"])){
    ...
    if (!in_array(true, array_column($upload_results, 'error'))) {
        ...
        $stmt = $conn->prepare("UPDATE `expenses` SET `CampaignID`=?,`Description`=?,`Amount`=?,`ExpenseDate`=? WHERE `ExpenseID`=?");

        try {
            $stmt->execute([ $CampaignID, $Description, $Amount, $ExpenseDate, $ExpenseID  ]);
        } catch (Exception $e) {
        ...
        if (!isset($error)){
            header("location: expenses-read.php?ExpenseID=$ExpenseID");
        } else {
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            if(mysqli_num_rows($result) == 1){
                ...
                $CampaignID = htmlspecialchars($row["CampaignID"] ?? "");
					...
					$ExpenseDate = htmlspecialchars($row["ExpenseDate"] ?? "");
                ...
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post" enctype="multipart/form-data">
						...
						<div class="form-group">
                                            ...
                                            <input type="date" name="ExpenseDate" id="ExpenseDate" class="form-control" value="<?php echo @$ExpenseDate; ?>">
                                        </div>
                        ...
                        <hr>
                        <p>
                            <a href="expenses-read.php?ExpenseID=<?php echo $_GET["ExpenseID"];?>" class="btn btn-info"><?php translate('View Record') ?></a>
