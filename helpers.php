...
// Scan directories and find db_connection.php
function getConfigDirectories($baseDir, $excludedDirs = ['locales', 'templates']) {
    ...
    $configDirs = [];
    foreach ($dirs as $dir) {
        if (file_exists($dir . '/db_connection.php')) {
...
function exportAsCSV($data, $db_name, $tables_and_columns_names, $table_name, $conn, $debug = false) {

    // Find relations in table
    $relations = get_foreign_key_relations($conn, $db_name);
    ...
    $lines = [];
    foreach($data as $row) {
        $lines[] = extract_csv_data($row, $relations, $db_name, $tables_and_columns_names, $table_name, $conn, $debug);
    }
...
function extract_csv_data($row, $relations, $db_name, $tables_and_columns_names, $table_name, $conn, $debug = false) {
    // Init headers
    $line = [];
        ...
        if ($column_config['columnvisible']) {
            ...
            if ($relation !== NULL) {
                ...
                if ($value !== null) {
                    ...
                    $related_value = get_related_table_data($conn, $relation['table'], $primary, $value, $debug);
                } else {
...
function get_related_table_data($conn, $referenced_table_name, $primary, $value, $debug = false) {
...
function get_foreign_key_relations($conn, $db_name) {
    $sql = "
        SELECT
            TABLE_NAME,
            COLUMN_NAME,
